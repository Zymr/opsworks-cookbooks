...
describe_recipe 'nginx::default' do
  ...
  it 'creates nginx directory' do
    directory(node[:nginx-i2][:dir]).must_exist.with(:owner, 'root').and(:group, 'root').and(:mode, '755')
  end

  it 'creates log directory' do
    directory(node[:nginx-i2][:log_dir]).must_exist.with(:owner, node[:nginx-i2][:user]).and(:mode, '755')
  end

  it 'creates debian style nginx site directories' do
    %w{sites-available sites-enabled conf.d}.each do |dir|
      directory(File.join(node[:nginx-i2][:dir], dir)).must_exist.with(:owner, 'root').and(:group, 'root').and(:mode, '755')
    end
  end
  ...
  it 'creates nginx conf file' do
    file(File.join(node[:nginx-i2][:dir], 'nginx.conf')).must_exist.with(:owner, 'root').and(:group, 'root').and(:mode, '644')
  end

  it "creates a non-empty pid file" do
    file(node[:nginx-i2][:pid_file]).must_match(/^\d+$/)
  end

  it 'creates default site' do
    file(File.join(node[:nginx-i2][:dir], 'sites-available', 'default')).must_exist.with(:mode, '644').and(:owner, 'root').and(:group, 'root')
  end
